...
export default function LiveMap({ hubs, vehicles, shipments, routes = [], showPlannedRoutes = true, onShipmentClick, focus, className }: Props) {
  ...
  useEffect(() => {
    ...
    Object.values(hubs).forEach((h) => {
      ...
      el.innerHTML = `<div style="display:flex;align-items:center;gap:4px"><div style="width:10px;height:10px;border-radius:3px;background:#64ffda;box-shadow:0 0 8px #64ffda"></div><span style="font:600 10px Inter;color:#cfd8f5;text-shadow:0 0 3px #000">${h.city}</span></div>`
      const marker = new maplibregl.Marker({ element: el, anchor: 'left', offset: [-5, 0] })
